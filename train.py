...
from utils.generic_utils import (Progbar, save_checkpoint,
                                 save_best_model, load_config, lr_decay,
                                 count_parameters, check_update)  # get_commit_hash)#create_experiment_folder,remove_experiment_folder
...
def main(args):
    # Setup the dataset
    print('fuck9-0!')
    train_dataset = LJSpeechDataset(os.path.join(c.data_path, 'metadata.csv'),
    ...
    print('fuck10!')
    '''
    val_dataset = LJSpeechDataset(os.path.join(c.data_path, 'metadata.csv'),
                                  os.path.join(c.data_path, 'wavs'),
                                  c.r,
                                  c.sample_rate,
                                  c.text_cleaner,
    ...
    for epoch in range(0, c.epochs):
        ...
        print('fuck training!')
        val_loss = evaluate(model, criterion, criterion_st, train_loader, current_step)
        print('fuck validing!')

        best_loss = save_best_model(model, optimizer, val_loss, best_loss, OUT_PATH, current_step, epoch)
        print('fuck best!')
